...
const MAX_AVATAR_OUTPUT_BYTES = 120_000;
...
async function buildAvatarDataUrl(file: File): Promise<string> {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    if (output.byteLength <= MAX_AVATAR_OUTPUT_BYTES || quality <= 42) {
  ...
  const finalOutput = await basePipeline.clone().webp({ quality: 40, effort: 5 }).toBuffer();
