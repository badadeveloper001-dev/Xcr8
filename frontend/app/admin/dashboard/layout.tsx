  ...
  return <main className="lux-page min-h-screen px-4 py-5">
    <div className="mx-auto max-w-6xl">
      <header className="xcr8-panel mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl p-4">
        ...
        <div className="flex items-center gap-2"><ThemeToggle /><button onClick={() => { sessionStorage.removeItem("xcr8-admin-access"); router.replace("/admin"); }} className="rounded-xl border border-white/10 px-3 py-2 text-xs text-slate-300"><LogOut size={14} className="mr-1 inline" />Sign out</button></div>
      </header>
