...
if [[ $# -lt 1 ]]; then
  ...
  echo "Example: $0 https://your-xcr8-domain.example/api/v1"
  exit 1
fi
...
echo "${HEALTH}" | "${PYTHON_BIN}" -c 'import json,sys; data=json.load(sys.stdin); assert data.get("status")=="ok"; print("health=ok")'

echo "[2/6] Database health"
HEALTH_DB="$(curl -fsS "${API_BASE_URL}/health/db")"
echo "${HEALTH_DB}" | "${PYTHON_BIN}" -c 'import json,sys; data=json.load(sys.stdin); assert data.get("database")=="ok"; print("db=ok")'
...
USER_ID="$(echo "${SIGNUP}" | "${PYTHON_BIN}" -c 'import json,sys; d=json.load(sys.stdin); print(d["user_id"])')"
...
POST_ID="$(echo "${DRAFT}" | "${PYTHON_BIN}" -c 'import json,sys; d=json.load(sys.stdin); print(d["post_id"])')"
echo "post_id=${POST_ID}"

echo "[6/7] Schedule and dashboard"
SCHEDULED_FOR="$("${PYTHON_BIN}" -c 'from datetime import datetime, timedelta, timezone; print((datetime.now(timezone.utc)+timedelta(hours=2)).isoformat())')"
...
echo "${DASHBOARD}" | "${PYTHON_BIN}" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("drafts",0) >= 0; assert d.get("scheduled",0) >= 1; print("dashboard=ok")'

echo "[7/7] AI usage analytics"
AI_USAGE="$(curl -fsS "${API_BASE_URL}/analytics/ai-usage/${USER_ID}")"
echo "${AI_USAGE}" | "${PYTHON_BIN}" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("total_generations",0) >= 1; assert "most_used_template" in d; print("ai_usage=ok")'
